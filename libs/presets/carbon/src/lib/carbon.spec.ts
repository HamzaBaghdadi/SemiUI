...
describe('Carbon preset', () => {
  const { root, dark } = buildThemeVars(Carbon);
  ...
  it('wires component tokens through the semantic layer rather than to literals', () => {
    ...
    // Carbon squares the whole library with a single radius decision.
    expect(vars['--semiui-radius-md']).toBe('0');
    expect(vars['--semiui-comp-button-radius']).toBe('var(--semiui-radius-md)');
    expect(vars['--semiui-typography-font-family']).toBe('"IBM Plex Sans", sans-serif');
  });

  it('resolves its primary all the way down to its own primitive', () => {
    expect(resolvePresetToken(Carbon, 'primary')).toBe('#0f62fe');
    expect(resolvePresetToken(Carbon, 'components.button.variants.primary.background')).toBe('#0f62fe');
